...
class Discriminator:
    ...
    def __call__(self, tensor_in, condition=None, training=None):
        ...
        with tf.variable_scope(self.name, reuse=tf.AUTO_REUSE):
            # genre embedding
            y = tf.layers.dense(condition, 16)
            y = tanh(y)

            y = tf.expand_dims(tf.expand_dims(tf.expand_dims(y, 1), 1), 1)
            ...
            with tf.variable_scope('shared'):
                ...
                # fuse genre embeeding
                y = tf.tile(y, [1, 4, 4, 3, 1])
                h = tf.concat((h, y), -1)
                
                h = conv_layer(h, 256, (1, 4, 3), (1, 4, 3))         # 4, 1, 1
        
#             # fuse genre embedding
#             y = tf.tile(y, [1, 4, 1, 1, 1])
#             h = tf.concat((h, y), -1)
