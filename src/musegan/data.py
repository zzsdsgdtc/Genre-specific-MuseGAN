...
# --- Tensorflow Dataset -------------------------------------------------------
def _gen_data(data, labels=None, wrong_labels=None):
    """Data Generator."""
    if labels is None:
        for item in data:
            if np.issubdtype(data.dtype, np.bool_):
        ...
        for i, item in enumerate(data):
            if np.issubdtype(data.dtype, np.bool_):
                yield (item * 2. - 1., labels[i], wrong_labels[i])
            else:
                yield (item, labels[i], wrong_labels[i])

def get_dataset(data, labels=None, wrong_labels=None, batch_size=None, data_shape=None,
                use_random_transpose=False, num_threads=1):
    """Create  and return a tensorflow dataset from an array."""
    if labels is None:
#         print("gen_data: " + str(data.shape))
...
#                 num_parallel_calls=num_threads)
        dataset = dataset.map(
            lambda pianoroll, label, wrong: (set_pianoroll_shape(
                pianoroll, data_shape), set_label_shape(label), set_label_shape(wrong)),
            num_parallel_calls=num_threads)
