...
def qualified_note_rate(tensor, threshold=6):
...
def get_save_metric_ops(tensor, beat_resolution, result_dir, suffix=None):
    ...
    def _save_array(array, name):
        """Save the input array."""
#         suffix_ = step if suffix is None else suffix
        filepath = os.path.join(
            result_dir, name, '{}_{}.npy'.format(name, suffix))
    ...
    for key, value in metric_ops.items():
        save_metric_ops[key] = tf.py_func(
            lambda array, k=key: _save_array(array, k),
            [value], tf.int32)
