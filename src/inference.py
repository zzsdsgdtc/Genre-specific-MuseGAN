...
def parse_arguments():
    ...
    parser.add_argument('--genre', type=str, default="jazz",
                        help="The genre you want to generate")
...
def main():
    ...
    params, config = setup()
    if config["genre"] == "jazz":
        config["genre_vec"] = [1, 0, 0]
    elif config["genre"] == "rock":
        config["genre_vec"] = [0, 1, 0]
    else:
        config["genre_vec"] = [0, 0, 1]
    ...
    with tf.Session(config=tf_config) as sess:
        ...
        for i in range(config['runs']):
            ...
            if params['is_conditional']:
                feed_dict_sampler[placeholder_y] = np.array([config["genre_vec"]] * (config['rows'] * config['columns']))
