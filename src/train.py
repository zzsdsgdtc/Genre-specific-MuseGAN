...
def load_training_data(params, config):
    ...
    if params['is_conditional']:
        ...
        labels = load_data(config['label_source'], config['label_filename'])
        wrong = load_data(config['label_source'], config['wrong_label_filename'])
    ...
    dataset = get_dataset(
        data, labels,wrong, config['batch_size'], params['data_shape'],
        config['use_random_transpose'], config['n_jobs'])

    # Create iterator
    if params['is_conditional']:
        train_x, train_y, train_y_w = dataset.make_one_shot_iterator().get_next()
    else:
        train_x, train_y, train_y_w = dataset.make_one_shot_iterator().get_next(), None, None

    return train_x, train_y, train_y_w
...
def load_or_create_samples(params, config):
    ...
    return sample_x, np.array([[1, 0, 0]] * (config['sample_grid'][0] * config['sample_grid'][1])), sample_z
...
def main():
    ...
    # ================================== Data ==================================
    # Load training data
    train_x, train_y, train_y_w = load_training_data(params, config)
    ...
    if params['is_accompaniment']:
        ...
        train_nodes = model(
            x=train_x, y=train_y, w=train_y_w, mode='train', params=params, config=config)
    ...
    with tf.train.MonitoredTrainingSession(
        ...
        # Training iteration
        for i in range(step, config['steps']):
            ...
            if ((config['save_samples_steps'] > 0)
                ...
                if params['is_conditional']:
                     feed_dict_sampler[placeholder_y] = sample_y
                if step < 3000:
                    sess.run(
            ...
            if ((config['evaluate_steps'] > 0)
                ...
                if params['is_accompaniment']:
                    feed_dict_evaluation[placeholder_c] = np.expand_dims(
                        sample_x[..., params['condition_track_idx']], -1)
                if params['is_conditional']:
                    feed_dict_evaluation[placeholder_y] = sample_y
